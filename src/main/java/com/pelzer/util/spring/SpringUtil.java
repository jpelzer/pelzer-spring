...
/**
 * Utility class for interfacing with Spring. Looks for any bean definition file
 * beginning with the word "beans". 
 */
...
public final class SpringUtil implements BeanFactoryAware, ApplicationContextAware {
  ...
  /** Spring config. Essentially the name of the beans XML file. */
  public static final String[] CONTEXT_DEFINITION         = new String[] { "classpath*:beans.xml", "classpath*:applicationContext.xml" };
  
  static boolean               hasGetSpringBeanBeenCalled = false;
  
  static {
    // Hack to get the system to turn down the logging for these chatty classes
    Logging.getLogger("com.google.code.morphia.query.QueryImpl");
    Logging.getLogger("com.google.code.morphia.mapping.Mapper");
    Logging.getLogger("com.google.code.morphia.DatastoreImpl");
  }
  ...
  public static <T extends Object> T getSpringBean(final Class<T> beanClass) {
    return getSpringBean(CONTEXT_DEFINITION, beanClass);
  }
